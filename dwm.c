...
int fakesignal(void) {
  ...
  numsegments =
      split(rootname + sizeof(prefix) - 1, sep, segments, sizeof(segments));
 ...
 */
void swal(Client *swer, Client *swee, int manage) {
  Client *c, **pc;
 ...
 */
void swalmanage(Swallow *s, Window w, XWindowAttributes *wa) {
  Client *swee, *swer;
  XWindowChanges wc;
  ...
  grabbuttons(swee, 0);
  XChangeProperty(dpy, root, netatom[NetClientList], XA_WINDOW, 32,
                  PropModeAppend, (unsigned char *)&(swee->win), 1);
...
void swalmouse(const Arg *arg) {
  ...
  if (XGrabPointer(dpy, root, False, ButtonPressMask | ButtonReleaseMask,
                   GrabModeAsync, GrabModeAsync, None, cursor[CurSwal]->cursor,
 ...
 */
void swalstop(Client *swee, Client *root) {
  Client *swer;
